...
class PopularMovieListBloc extends Bloc<PopularMovieListEvent, PopularMovieListState> {
  ...
  }) : super(PopularMovieListState.initial()) {
    on<GetPopularMovieListEvent>(_getPopularMovieList);
  }

  Future<void> _getPopularMovieList(
    GetPopularMovieListEvent event,
    Emitter<PopularMovieListState> emit,
  ) async {
    emit(state.copyWith(status: MovieListStatus.loading));
    try {
      String? errorMsg;
      final MovieList movieList = await movieRepository.getPopularList().onError((error, stackTrace) {
